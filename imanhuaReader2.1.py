...
'''
	...
	本次修改：
	0、增加了显示当前页数和章节的功能，为实现书签做准备。
	1、修正了在某章首页点上一页时，返回上一章第一页的BUG。
	2、修正了Chapter翻页到下一张时抛出indexError的BUG。
	3、实现了翻页后自动定位至图片开始的功能 （enVisible）
'''
	...
	def generateAddr(self,page,chapter):
		if page >= len(self.imgList):
			...
			if chapter > 1:
				...
				self.setDictReady()
				self.nowPage = len(self.imgList) - 1
			else:
				self.nowChapter = 0
				self.nowPage    = 0
				self.setDictReady()
...
class Chapter(QWidget):
	...
	def __init__(self,parent = None):
		...
		layout.addWidget(self.showPage,0,1)
		layout.addWidget(self.showChap,0,2)
		self.layout = layout
		self.setLayout(layout)			
	...
	def showImg(self):
		self.showThePage()
		self.showTheChap()
		self.imgScrol.ensureVisible(0,0) # 关键！！意思是保证图片中某个坐标能被看到
	...
	def showThePage(self):
		self.showPage.setText(unicode(self.page + 1) + u'页')
		
	def showTheChap(self):
		self.showChap.setText(manhuainx[self.chapter])
